...
BOOST_AUTO_TEST_CASE(user_types)
{
  ...
  // Across hierarchy
  BOOST_TEST_REQUIRE((!runUpcastAllowed<MCConcreteVirtual_B,MCConcreteVirtual_A>()));
...
BOOST_AUTO_TEST_CASE(user_type_variables)
{
  ...
  BOOST_TEST_REQUIRE( (!art::detail::upcastAllowed(typeid(refToBase), typeid(MCConcrete))) );

  // Virtual
  MCConcreteVirtual_A actualConcreteVirtual{};
  MCBaseVirtual& refToBaseVirtual = actualConcreteVirtual;
  BOOST_TEST_REQUIRE( (art::detail::upcastAllowed(typeid(refToBaseVirtual), typeid(MCConcreteVirtual_A))) );
  BOOST_TEST_REQUIRE( (!art::detail::upcastAllowed(typeid(refToBaseVirtual), typeid(MCConcreteVirtual_B))) );
}
